...
$$ 
log(\frac{Likely_i}{1-Likely_i}) = \alpha_j + \sum_{a \in A}\beta_a(Age_i=a) +\sum_{r \in R}^{}\beta_r(Race/Ethnicity_i=r) + \sum_{s \in S}^{}\beta_s(Sex_i=s) \\
...
x <- sjPlot::tab_model(glmer.fit1, show.re.var= TRUE,dv.labels= "Effect of Class and Course Materials", p.style = "numeric_stars", file = "results.html")
ws <- webshot("results.html", "results.png")
```

![Age Bin Random Slope Model](results.png)
