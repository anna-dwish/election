...
In general, we see an increasing trend with age for *Non-Hispanic American Indian*, *Non-Hispanic Black*, and *Non-Hispanic White* groups. This is slightly less profound for *Hispanic Any Race*, *Non-Hispanic Asian*, and *Non-Hispanic Mixed* groups.
...
$$ 
\begin{aligned}
log(\frac{P_i(Likely)}{1-P_i(Likely)}) = \alpha_j + \sum_{a \in A}\beta_a(Age_i=a) +\sum_{r \in R}^{}\beta_r(Race/Ethnicity_i=r) + \sum_{s \in S}^{}\beta_s(Sex_i=s) \\
a_j \sim  N(\gamma_0 + \gamma_1\mu_j, \tau^2) \\
A = \{ 18-29,30-39,40-49,50-64,65+ \} \\
R = \{Hispanic.Any.Race, American.Indian, Asian, Black, Mixed, White\} \\
S = \{Female,Male\}
\end{aligned}
$$
...
glmer.fit.final <- glmer(cbind(Likely, Unlikely) ~ age_bin + race_ethnicity + gender + (1 + age_bin | county_desc),
                    data = fold_df,
                    family = binomial,
                    control = glmerControl(optimizer = "optimx",
                                           calc.derivs = FALSE,
...
test.glmer.probs <- predict(glmer.fit.final, validation_set,  type = "response")
...
The results of our final model can be viewed in Table ** along with a heatmap of the intercepts by county. The accuracy and F1-score on our test set was `r round(test_acc,3)` and `r round(test_f1,3)` respectively. Our ROC curve also provides evidence this model fits well with an AUC of `r round(test_AUC,3)`.
...
Now, we will consider our model's ability to predict on North Carolina's 2014 Senate election. For our out of sample predictions, we achieved an `r round(election2014.acc,3)*100`% accuracy. This suggests...
...
ws <- webshot("results_reaged.html", "results_reaged.png")
```

![Table 2: Age Bin Sensitivity Models](results_reaged.png)
