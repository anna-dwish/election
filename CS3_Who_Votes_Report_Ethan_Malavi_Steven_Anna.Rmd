...
The North Carolina voter history dataset shows how each registered voter in North Carolina, uniquely identified by a voter registration number, voted in the 2016 presidential and 2018 congressional elections. As not all voters participated in both elections, we designated only those who voted in 2016 as “likely” voters for the 2020 election, and disregarded those who voted in 2018. This decision was made because both 2016 and 2020 are presidential election years, while there is typically lower turnout for midterm elections [[2]][Bibliography]. 
  ...
  mutate(race_ethnicity = case_when(
    ...
    race_ethnicity == "Non-Hispanic Black" ~ paste0("Non-Hispanic Black (n=",
                                                   who_votes_data %>%
                                                     ...
                                                     summarise(sum(sums)),
                                                   ")"),
    race_ethnicity == "Non-Hispanic Mixed" ~ paste0("Non-Hispanic Mixed (n=",
                                                   who_votes_data %>%
                                                     mutate(sums = Likely + Unlikely) %>%
...
<<<<<<< HEAD
The results of our final model can be viewed in Table 1 in our Appendix. Figure 3 is a heatmap of the intercepts by county. While the intercepts appear to generally capture relative trends, these intercepts appear to be more homogeneous than the true percent of likely voters. This is likely a consequence of our assumption that they stem from the same distribution. The average accuracy and average F1-score on our test sets were `r cv_df$Accuracy` and `r cv_df$F1` respectively. Our ROC curve also provides evidence this model fits well with an average AUC of `r cv_df$AUC`.
=======
The results of our final model can be viewed in Table 1 in our Appendix, and Figure 3 is a heatmap of the random intercepts by county. While the intercepts appear to generally capture relative trends, these intercepts appear to be more homogeneous than the true percent of likely voters. This is likely a consequence of our assumption that they stem from the same distribution. The average accuracy and average F1-score on our test sets were `r cv_df$Accuracy` and `r cv_df$F1` respectively. Our ROC curve also provides evidence this model fits well with an average AUC of `r cv_df$AUC`. These evaluation metrics indicate that our model does a reasonable job of predicting who votes in North Carolina.  
>>>>>>> ce81fc965c3c51e9427b0a3d21f1aeb74f5af934
...
```
```{r}
row.names(final.model.fixed.results) <- final.model.fixed.results$id
```



```{r}
gender <- round(exp(final.model.fixed.results[11,][,1]), 1)
```

With respect to fixed effects, we see that, with all else held constant, the odds of a female voting in 2020 are `r gender` that of a male. Understanding the effects of race/ethnicity and age are not as straightfoward, as the interaction between the two terms indicates that one cannot be interpreted in isolation of the other. We will focus on an interesting group of voters, 18-29 year olds. 

```{r}
white <- round(exp(final.model.fixed.results["age_bin18-29:race_ethnicityNon-Hispanic White",1] + final.model.fixed.results["race_ethnicityNon-Hispanic White",1]), 2)
white_lower <- round(exp(final.model.fixed.results["age_bin18-29:race_ethnicityNon-Hispanic White",5] + final.model.fixed.results["race_ethnicityNon-Hispanic White",5]), 2)
white_upper <- round(exp(final.model.fixed.results["age_bin18-29:race_ethnicityNon-Hispanic White",6] + final.model.fixed.results["race_ethnicityNon-Hispanic White",6]), 2)


mixed <- round(exp(final.model.fixed.results["age_bin18-29:race_ethnicityNon-Hispanic Mixed",1] + final.model.fixed.results["race_ethnicityNon-Hispanic Mixed",1]), 2)
mixed_lower <- round(exp(final.model.fixed.results["age_bin18-29:race_ethnicityNon-Hispanic Mixed",5] + final.model.fixed.results["race_ethnicityNon-Hispanic Mixed",5]), 2)
mixed_upper <- round(exp(final.model.fixed.results["age_bin18-29:race_ethnicityNon-Hispanic Mixed",6] + final.model.fixed.results["race_ethnicityNon-Hispanic Mixed",6]), 2)


black <- round(exp(final.model.fixed.results["age_bin18-29:race_ethnicityNon-Hispanic Black",1] + final.model.fixed.results["race_ethnicityNon-Hispanic Black", 1]), 2)
black_lower <- round(exp(final.model.fixed.results["age_bin18-29:race_ethnicityNon-Hispanic Black",5] + final.model.fixed.results["race_ethnicityNon-Hispanic Black", 5]), 2)
black_upper <- round(exp(final.model.fixed.results["age_bin18-29:race_ethnicityNon-Hispanic Black",6] + final.model.fixed.results["race_ethnicityNon-Hispanic Black", 6]), 2)

asian <- round(exp(final.model.fixed.results["age_bin18-29:race_ethnicityNon-Hispanic Asian",1] + final.model.fixed.results["race_ethnicityNon-Hispanic Asian", 1]), 2)
asian_lower <- round(exp(final.model.fixed.results["age_bin18-29:race_ethnicityNon-Hispanic Asian",5] + final.model.fixed.results["race_ethnicityNon-Hispanic Asian", 5]), 2)
asian_upper <- round(exp(final.model.fixed.results["age_bin18-29:race_ethnicityNon-Hispanic Asian",6] + final.model.fixed.results["race_ethnicityNon-Hispanic Asian", 6]), 2)

amind <- round(exp(final.model.fixed.results["age_bin18-29:race_ethnicityNon-Hispanic American Indian",1] + final.model.fixed.results["race_ethnicityNon-Hispanic American Indian", 1]), 2)
amind_lower <- round(exp(final.model.fixed.results["age_bin18-29:race_ethnicityNon-Hispanic American Indian",5] + final.model.fixed.results["race_ethnicityNon-Hispanic American Indian", 5]), 2)
amind_upper <- round(exp(final.model.fixed.results["age_bin18-29:race_ethnicityNon-Hispanic American Indian",6] + final.model.fixed.results["race_ethnicityNon-Hispanic American Indian", 6]), 2)

```

```{r}
Type <- c("White (Non Hispanic)", "Mixed (Non Hispanic)", "Black (Non Hispanic)", "Asian (Non Hispanic)", "American Indian (Non Hispanic)")
...
The table above shows, for 18-29 years old with county and gender held constant, the multiplicative odds of being a likely voter compared to the baseline of Hispanic (all races). As can be seen, for White and Black individuals, the odds of being a likely voter multiply by of `r white` and `r black` respectively. Thus, these individuals are more likely to vote than their hispanic counterparts. Conversely, for 18-29 year old Asians, American Indians, and Mixed individuals, the odds of being a likely voter multiple by factors of `r asian`, `r amind` and `r mixed` respective. Thus, these individuals are less likely to vote than their hispanic counterparts. It is important to note that these trends are specific to 18-29 year olds in North Carolina. 
