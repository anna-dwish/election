...
$$ 
\begin{aligned}
log(\frac{P_{ij}}{1-P_{ij}}) = \alpha_{j} + \alpha_{1} * I(Gender_{ij}=Male &) + \sum_{a = 2}^5\alpha_{2a}*I(Age_{ij}=a) + \  \\
...
We compared this model to one without an interaction between *Age* and *Race/Ethnicity*, one with a random intercept for county and random effect slope for each of the three demographic covariates, and to models with pairwise interactions between each demographic covariate (in addition to the one between *Age* and *Race/Ethnicity*). In order to compare the fits of these models, we used estimated $F1-scores$ from 5-fold cross-validations, $ROC$ curves, and $F1-scores$ on the out of sample predictions on 2012 elections. Our performance on the 2012 elections will help quantify how well our model can perform on presidential election years outside of the dataset's election. Additionally, once we had our final model from these comparisons, we further examined its binned residual plots to better understand the drawbacks of our final model.
...
create_model <- function(model_formula, election_data, perform_cv=TRUE){
  model_results = list()
  results_idx = 1
...
model.random.county.intercept.interaction.age.race.text <- "cbind(Likely,Unlikely) ~ age_bin + race_ethnicity + gender + party_cd + age_bin * race_ethnicity + (1|county_desc)"


model.ri.county.intrxn.age.race.formula <- formula(model.random.county.intercept.interaction.age.race.text)
model.ri.county.intrxn.age.race.results <- create_model(model.ri.county.intrxn.age.race.formula, fold_df)

cv_df <- model.ri.county.intrxn.age.race.results[[1]]
glmer.fit.final <- model.ri.county.intrxn.age.race.results[[2]]
final.model.fixed.results <- model.ri.county.intrxn.age.race.results[[3]]
```
...
arm::binnedplot(x=cv_plot(glmer.fit.final, fold_df)[[4]],y=residuals(glmer.fit.final, "pearson", scaled = TRUE),
...
model.random.intercept.no.interaction.text <- "cbind(Likely,Unlikely) ~ age_bin + race_ethnicity + party_cd + gender + (1 |county_desc)"
model.ri.county.no.intrxn.formula <- formula(model.random.intercept.no.interaction.text)
model.ri.county.no.intrxn.results <- create_model(model.ri.county.no.intrxn.formula, fold_df, perform_cv = FALSE)
...
arm::binnedplot(x=model.ri.county.no.intrxn.cm[[4]],y=residuals(model.ri.county.no.intrxn.results[[1]], "pearson", scaled = TRUE),
                xlab="Predicted Probabilities", 
                ylab="Binned Residuals",
...
## Random Age Slope No Interaction
...
model.random.slope.age.no.intrxn.text <- "cbind(Likely,Unlikely) ~ age_bin + race_ethnicity + party_cd + gender + (1 + age_bin|county_desc)"
model.rs.age.no.intrxn.formula <- formula(model.random.slope.age.no.intrxn.text)
model.rs.age.no.intrxn.results <- create_model(model.rs.age.no.intrxn.formula, fold_df, perform_cv = FALSE)
model.rs.age.no.intrxn.cm <- cv_plot(model.rs.age.no.intrxn.results[[1]], fold_df)
model.rs.age.no.intrxn.cm[[3]]
```
...
cv_plot(model.rs.age.no.intrxn.results[[1]], election2012)[[3]]
```
...
model.random.slope.age.intrxn.text <- "cbind(Likely,Unlikely) ~ age_bin * race_ethnicity + gender + party_cd + (1 + age_bin|county_desc)"
model.rs.age.intrxn.formula <- formula(model.random.slope.age.intrxn.text)
model.rs.age.intrxn.results <- create_model(model.rs.age.intrxn.formula, fold_df, perform_cv = FALSE)
model.rs.age.intrxn.cm <- cv_plot(model.rs.age.intrxn.results[[1]], fold_df)
model.rs.age.intrxn.cm[[3]]
```

```{r oos random age slope, fig.height=3, fig.align="center",warning=F,message=F,echo=FALSE,results='hide',fig.keep='all'}
model.rs.age.intrxn.oos.cm <- cv_plot(model.rs.age.intrxn.results[[1]], election2012)
cv_plot(model.rs.age.intrxn.results[[1]], election2012)[[3]]
```

```{r oos random age slope pander table}
pander(model.rs.age.intrxn.results[[2]], caption="Random County Intercept and Age Slope Results")
```

```{r random age slope residual plot,fig.height=3,fig.align="center", warning=FALSE,echo=FALSE}
arm::binnedplot(x=model.rs.age.intrxn.cm[[4]],y=residuals(model.rs.age.intrxn.results[[1]], "pearson", scaled = TRUE),
                xlab="Predicted Probabilities", 
                ylab="Binned Residuals",
                main = "Figure 7: Random County Intercept and Age Slope, Age and RaceEthnicity Interaction",
                cex.main=0.9, cex.lab=0.8)
```

```{r fig.width=8, fig.height=6}
merTools::plotREsim(merTools::REsim(model.rs.age.intrxn.results[[1]])) %>% +
  labs(x='County', y='Value', title='Interval Estimates of Random Effects by County') +
  geom_hline(aes(yintercept=0), color='orange', alpha=.5) +
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
        strip.text.x = element_blank(),
...
model.random.slope.race.ethnicity.intrxn.text <- "cbind(Likely,Unlikely) ~ age_bin * race_ethnicity + party_cd + gender + (1 + race_ethnicity|county_desc)"
model.rs.race.ethnicity.intrxn.formula <- formula(model.random.slope.race.ethnicity.intrxn.text)
model.rs.race.ethnicity.intrxn.results <- create_model(model.rs.race.ethnicity.intrxn.formula, fold_df, perform_cv = FALSE)
...
model.random.slope.gender.intrxn.text <- "cbind(Likely,Unlikely) ~ age_bin * race_ethnicity + party_cd + gender + (1 + gender|county_desc)"
model.rs.gender.intrxn.formula <- formula(model.random.slope.gender.intrxn.text)
model.rs.gender.intrxn.results <- create_model(model.rs.gender.intrxn.formula, fold_df, perform_cv = FALSE)
model.rs.gender.intrxn.cm <- cv_plot(model.rs.gender.intrxn.results[[1]], fold_df)
model.rs.gender.intrxn.cm[[3]]
```
...
model.intrxn.age.gender.text <- "cbind(Likely,Unlikely) ~ age_bin * race_ethnicity + party_cd + age_bin * gender + (1 | county_desc)"
model.intrxn.age.gender.formula <- formula(model.intrxn.age.gender.text)
model.intrxn.age.gender.results <- create_model(model.intrxn.age.gender.formula, fold_df, perform_cv = FALSE)
model.intrxn.age.gender.cm <- cv_plot(model.intrxn.age.gender.results[[1]], fold_df)
model.intrxn.age.gender.cm[[3]]
```
...
cv_plot(model.intrxn.age.gender.results[[1]], election2012)[[3]]
```
...
model.intrxn.re.gender.text <- "cbind(Likely,Unlikely) ~ age_bin * race_ethnicity + party_cd + race_ethnicity * gender + (1 | county_desc)"
model.intrxn.re.gender.formula <- formula(model.intrxn.re.gender.text)
model.intrxn.re.gender.results <- create_model(model.intrxn.re.gender.formula, fold_df, perform_cv = FALSE)
model.intrxn.re.gender.intrxn.cm <- cv_plot(model.intrxn.re.gender.results[[1]], fold_df)
model.intrxn.re.gender.intrxn.cm[[3]]
```
...
### Race_Ethnicity * Party Without Race_Ethnicity * Age_Bin
...
model.intrxn.re.party.only.text <- "cbind(Likely,Unlikely) ~ gender + age_bin + race_ethnicity * party_cd + (1 | county_desc)"
model.intrxn.re.party.only.formula <- formula(model.intrxn.re.party.only.text)
model.intrxn.re.party.only.results <- create_model(model.intrxn.re.party.only.formula, fold_df, perform_cv = FALSE)
model.intrxn.re.party.only.cm <- cv_plot(model.intrxn.re.party.only.results[[1]], fold_df)
model.intrxn.re.party.only.cm[[3]]
```
...
cv_plot(model.intrxn.re.party.only.results[[1]], election2012)[[3]]
```


### Race_Ethnicity * Party 

```{r intrxn race party, fig.height=3, fig.align="center",warning=F,message=F,echo=FALSE,results='hide',fig.keep='all',cache=TRUE}
model.intrxn.re.party.text <- "cbind(Likely,Unlikely) ~ gender + age_bin * race_ethnicity + race_ethnicity * party_cd + (1 | county_desc)"
model.intrxn.re.party.formula <- formula(model.intrxn.re.party.text)
model.intrxn.re.party.results <- create_model(model.intrxn.re.party.formula, fold_df, perform_cv = FALSE)
model.intrxn.re.party.cm <- cv_plot(model.intrxn.re.party.results[[1]], fold_df)
model.intrxn.re.party.cm[[3]]
```
...
cv_plot(model.intrxn.re.party.results[[1]], election2012)[[3]]
```
...
model_comparisons <- data.frame(Formula=c(model_label(model.random.county.intercept.interaction.age.race.text),
                     model_label(model.random.intercept.no.interaction.text),
                     model_label(model.random.slope.age.no.intrxn.text),
                     model_label(model.random.slope.age.intrxn.text),
                     model_label(model.random.slope.race.ethnicity.intrxn.text),
                     model_label(model.random.slope.gender.intrxn.text),
                     model_label(model.intrxn.age.gender.text),
                     model_label(model.intrxn.re.gender.text),
                     model_label(model.intrxn.re.party.only.text),
                     model_label(model.intrxn.re.party.text)),
           In.Sample.F1.Score=c(final.insample.results[[2]],
                                model.ri.county.no.intrxn.cm[[2]],
                                model.rs.age.no.intrxn.cm[[2]],
                                model.rs.age.intrxn.cm[[2]],
                                model.rs.race.ethnicity.intrxn.cm[[2]],
                                model.rs.gender.intrxn.cm[[2]],
                                model.intrxn.age.gender.cm[[2]],
                                model.intrxn.re.gender.intrxn.cm[[2]],
                                model.intrxn.re.party.only.cm[[2]],
                                model.intrxn.re.party.cm[[2]]),
           Out.Sample.F1.Score=c(election2012.results[[2]],
                                 oos.no.intrxn[[2]],
                                 model.rs.age.no.intrxn.oos.cm[[2]],
                                 model.rs.age.intrxn.oos.cm[[2]],
                                 model.rs.race.ethnicity.intrxn.oos.cm[[2]],
                                 model.rs.gender.intrxn.oos.cm[[2]],
                                 model.intrxn.age.gender.oos.cm[[2]],
                                 model.rs.re.gender.intrxn.oos.cm[[2]],
                                 model.intrxn.re.party.only.oos.cm[[2]],
                                 model.intrxn.re.party.oos.cm[[2]]))
pander(model_comparisons, caption="Model Performances In Sample, 2016, and Out Sample, 2012, Elections")
```

## 2012 Data

```{r EDA 2012, fig.width=9, fig.align="center", warning=F,message=F,echo=FALSE,results='hide',fig.keep='all'}
# This code chunk prefaces some of the other package load-ins because of conflicts with select fuctionality 
election2012 %>% 
  mutate(sums = Likely + Unlikely) %>%
  mutate(race_ethnicity = case_when(
  ...
  labs(x = expression("Age Group"), 
       y= ("Proportion of Likely Voters"), 
       title = "Likely Voters Across Age Groups, Race/Ethnicity, and Sex in 2012", 
       caption="Figure 10") +
  theme(plot.caption = element_text(hjust = 0.5, vjust = -0.5, size = 12)) +
  guides(fill=guide_legend(title="Sex"))
```

This shows how a generally higher percentage of women went to the polls in 2012, likely because of Trump's divisive and derogatory rhetoric towards women, along with the stigma against Hillary Clinton. Struggling to predict Unlikely voters well because a lot of people didn't show up to the polls in 2016.

## Age Bin Sensitivity

```{r age bin sensitivity df, warning=F,message=F,echo=FALSE,results='hide',eval=F}
...
```{r age bin sensitivity analysis model results, results="hide", warning=F,message=F, cache=TRUE,eval=F}
fold_df <- groupdata2::fold(who_votes_data_reaged, k = 5, cat_col = c("age_bin", "race_ethnicity", "gender","county_desc")) %>%
  dplyr::mutate(vote = ifelse(Likely/(Unlikely+Likely) > 0.5, 1, 0),
                vote = ifelse(is.nan(Likely/(Unlikely+Likely)s), 0, vote),
                vote = dplyr::case_when(
                  Likely == 0 & Unlikely == 0 ~ 0,
                  TRUE ~ vote
                )) 
