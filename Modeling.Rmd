...
president_polls <- president_polls %>%
  filter(state %in% c("AR", "CO", "FL", "GA", "IA", "ME", "MI", "NC", "OH", "PA", "TX", "WI", "MN", "NV", "NH"))
...
ustates <- c(unique(president_polls$state))
State_Abbrev[!(State_Abbrev %in% ustates)]
```
...
vote_share_2016 <- unique(vote_share_2016) %>%
  mutate(clinton_percent = case_when(
    ...
    #president_polls.state == "US" ~ 48/(48 + 46),
...
jags_data <- list("y","t", "r", "N_polls", "N_states", "N_days", "h", "I_states")
...
jags_sims_mv <- jags(data = jags_data,model.file = model2,parameters.to.save = c("theta","Sigma",
                                                                                "p","sigma2_y"),
                  n.iter = 100)
```
