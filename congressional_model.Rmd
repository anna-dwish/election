...
```{r}
read_csv("grouped_voter_only_data_2016.csv")
```

```{r}
#??? why can't it split into folds
...
```{r}
smp_size <- floor(0.75 * nrow(grouped_voter_data))

## set the seed to make your partition reproducible
set.seed(123)
train_ind <- sample
(seq_len(nrow(grouped_voter_data)), size = smp_size)

train <- grouped_voter_data[train_ind, ]
test <- grouped_voter_data[-train_ind, ]
```


```{r}
glmer.fit.congressional <- glmer(cbind(Democrat, Republican) ~ age_bin + race_ethnicity + gender + party_cd + party_cd * race_ethnicity + (1|county_desc) + (1|cong_dist_abbrv),
                         data = train,
                         family = binomial,
...
```{r}
test <- test %>%
  mutate(vote = ifelse(Democrat/(Democrat+Republican) > 0.5, 1, 0),
...
```{r}
glmer.probs <- predict(glmer.fit.congressional,  test,  type = "response")
glmer.pred <- rep(0, nrow(test))
glmer.pred[glmer.probs > 0.5] <- 1
  
acc <-  mean(glmer.pred == test$vote)
f1 <- (caret::confusionMatrix(glmer.pred %>% as.factor(), test$vote %>% as.factor()))$byClass["F1"]
```


```{r cross validation, warning=FALSE, message=F, cache=TRUE}
set.seed(5934512)
fold_df <- groupdata2::fold(grouped_voter_data, k = 5, cat_col = c("age_bin", "race_ethnicity", "gender","county_desc")) %>%
...
create_model <- function(model_formula, grouped_voter_data, perform_cv=TRUE){
  model_results = list()
  results_idx = 1
  ...
  if (perform_cv){
    ...
    AUC_list = c()
    for (i in 1:5) {
      train <- grouped_voter_data %>% filter(.folds != i)
      test <- grouped_voter_data %>% filter(.folds == i)
