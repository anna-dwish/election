...
model_file <- generate_senate_model_text(senate_gamma_5_1_prior)
if (file.exists("senate_gamma_5_1_prior.Rds")) {
  senate_gamma_5_1_prior <- readRDS("senate_gamma_5_1_prior.Rds")
} else {
  senate_gamma_5_1_prior <- jags(data = jags_data_senate,  
                                     parameters.to.save = c("theta", "Sigma", 
                                                            "p","sigma2_y", "sigma2_0"),
                                     n.iter = 1000, #n.burnin = 25000, n.thin = 50, n.chains = 2,
                                     model.file = model_file)
  
  saveRDS(senate_gamma_5_1_prior, file = "senate_gamma_5_1_prior.Rds")
}
```

```{r}
# this function finds the probability that the senate flips!
#senate_gamma_5_1_prior <- readRDS("senate_gamma_2_2_prior.Rds")
n_iter <- dim(senate_gamma_5_1_prior$BUGSoutput$sims.list$theta[,,1])[1]
...
for (i in 1:n_iter){
  final_results_senate[i] = prediction_per_simulation_senate(senate_gamma_5_1_prior, i)
}
...
<<<<<<< HEAD
-------------------------IGONRE---------------------------------
Model WITH priors from last presidential election THIS SUCKS DON'T USE!
=======
#senate_gamma_5_1_prior$BUGSoutput$sims.list$theta[,14,1] %>% hist()
```

Model WITH priors from last presidential election
>>>>>>> ae9115b8d7889974159f01ab92d48f5fc1dc20f3
...
model_mv_senate_adj_priors <- function(){
  ...
  for(j in 1:N_states){
    ...
    theta[j,1] ~ dnorm(h[j], pow(sigma2_0,-2))
    #theta[j,1] ~ dnorm(mu0,pow(sigma2_0,-1))
  }
  nu_y ~ dunif(0,100)
  tau_y ~ dunif(0,100)
...
jags_data_senate_adj <- list(y=y_senate,t=t_senate,r=r_senate,
                         N_polls=N_polls_senate,N_states=N_states_senate,N_days=N_days_senate, I_states=I_states_senate,h=h)
jags_sims_mv_senate_adj_prior <- jags(data = jags_data_senate_adj,  model.file = model_mv_senate_adj_priors,parameters.to.save = c("theta","Sigma",
                                                                                                                                       "p","sigma2_y"),
                                      n.iter = 1500)
                                      n.iter = 1000, #n.burnin = 50000, n.thin = 75, n.chains = 2
                                      )
