...
senate <- senate_data %>%
  ...
  filter(state %in% c("Alabama", "Arizona*", "Colorado", "Georgia*", "Georgia", "Iowa", "Kansas", "Kentucky", "Maine", "Michigan", "Minnesota", "Mississippi", "Montana", "New Hampshire", "New Mexico", "North Carolina", "South Carolina", "Texas"))
```
...
senate_gamma_0.1_0.5_prior <- "
  ...
  sigma2_0 = 1/sigma2_0_inv
  sigma2_0_inv ~ dgamma(0.1, 0.5)
  "

model_file <- generate_senate_model_text(senate_gamma_0.1_0.5_prior)
if (file.exists("senate_gamma_0.1_0.5_prior.Rds")) {
  senate_gamma_0.1_0.5_prior <- readRDS("senate_gamma_0.1_0.5_prior.Rds")
} else {
  senate_gamma_0.1_0.5_prior <- jags(data = jags_data_senate,  
                                     parameters.to.save = c("theta", "Sigma", 
                                                            "p","sigma2_y", "sigma2_0"),
                                     n.iter = 1000, #n.burnin = 25000, n.thin = 50, n.chains = 2,
                                     model.file = model_file)
  
  saveRDS(senate_gamma_0.1_0.5_prior, file = "senate_gamma_0.1_0.5_prior.Rds")
}
```

```{r}
# this function finds the probability that the senate flips!
n_iter <- dim(senate_gamma_0.1_0.5_prior$BUGSoutput$sims.list$theta[,,1])[1]
...
for (i in 1:n_iter){
  final_results_senate[i] = prediction_per_simulation_senate(senate_gamma_0.1_0.5_prior, i)
}
