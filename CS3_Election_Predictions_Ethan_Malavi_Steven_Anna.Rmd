...
The 2020 U.S. elections, coupled with the unique social and economic turmoil corroborated by COVID-19, is documented to have globally political implications [[1]][Bibliography],[[2]][Bibliography]. Researchers have thus used prediction models to determine election outcomes. But often, with the unique voting patterns of certain demographics, making conclusive results requires additional focus on who votes and where. Therefore, in an attempt to predict the US presidential, senate-majority, electoral college, NC congressional, and NC senate election outcomes, our study explores the voting patterns of certain demographics and the characterization of uncertainty in our predicted results. In addition to determining the likely voters, when modeling how certain demographics vote, our findings illuminate the groups in a swing state like North Carolina that are hindered from civic engagement, whether by implicit or explicit means. This information can be instrumental to campaigns and activist groups that are seeking to bolster support and foster more politically minded communities. 
...
To model the outcome of the presidential election, we used an extension of the Linzer model, which takes a dynamic Bayesian approach to forecasting election results [[3]][Bibliography]. Our motivation for selecting this model is its ability to compound polling information over time and account for evolving preferences. While the Linzer model incorrectly predicted the results of the 2016 presidential election due to polling information inaccuracies, we hope to balance this reliance on the polling information by adjusting our priors to rely mo  re heavily on the 2016 election [[5]][Bibliography]. By June 9th, Biden had enough delegates to secure his nomination as the Democratic candidate, so we only included polls whose start date was after June 1st. These data were used to predict the outcomes of 15 swing states: Arizona, Colorado, Florida, Georgia, Iowa, Maine, Michigan, Minnesota,  New Hampshire, Nevada, North Carolina, Ohio, Pennsylvania, Texas, and Wisconsin [[4]][Bibliography], with a hierarchical prior on our model parameters. Further, our final model estimates the correlation structure between states, so more similar states pull information from one another.
...
Extract the 15 values of  $\theta_{\cdot 1}$, Biden’s estimated vote share on the day of the election from each swing state $i \in (1 \dots 15)$.
Initialize a variable, swing_electoral_votes, to 0. (electoral_votes = 0)
For each $\theta_{i 1}$ value
If it is greater than 50%, record the number of electoral votes from swing state $i$. add the number of electoral votes from that state to swing_electoral_votes.
Otherwise, don’t add to swing_electoral_votes.

Based on [SOURCE], Biden is expected to safely win 200 electoral votes from __ states. The sum of his expected electoral votes with the predicted electoral votes from the 15 swing states indicates the total number of estimated electoral votes he would win. Now, let electoral votes = 200 + swing_electoral_votes. These 200 votes come from states that Biden will safely win, as noted by several political experts (citation). Thus, electoral_votes represents the predicted number of electoral votes going to Joe Biden in the sample. If the number of estimated electoral votes is greater than 270, we predict that Biden wins the electoral college vote. Thus, from this model, we are able to predict the electoral college vote and the outcome of the presidential election.

## Results

From the algorithm above, we obtain, for each sample, the predicted number of electoral votes going to Biden. Computing the mean, 2.5% quantile, and 95% quantile of the predicted electoral votes across the 2000 samples, we see that Biden is predicted to win ____ (mean) electoral votes with a 95% confidence interval of (lower, upper). We further find that in __% of our samples, Biden is predicted to have over 270 electoral votes. Thus, we predict Biden will win with ___% probability. 

### Model Diagnostics

Traceplots for select parameters and quantiles for values can be found in the Appendix. To determine if our model has converged sufficiently, we first examine traceplots for each of the parameters. Traceplots with random scatter around a mean value indicates that the model has converged. $R_hat$ values provide an estimate of convergence based on the variance of an estimated parameter between chains, and the variance within a chain. The closer the 𝑅̂ value to 1, the better the model has converged. The traceplots show the chains have mixed well and $R_hat$ values close to 1, indicating that our model has sufficiently converged. 
